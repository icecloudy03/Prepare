...
object NativeUtil {
     ...
     * @Description: JNI基本压缩
     */
    fun compressBitmap(bit: Bitmap, fileName: String, optimize: Boolean) {
     ...
     */
    fun compressBitmap(image: Bitmap, filePath: String) {
     ...
     * @param file
     */
    fun compressImageToFile(bmp: Bitmap, file: File) {
        // 0-100 100为不压缩
        val options = 20
        val baos = ByteArrayOutputStream()
        // 把压缩后的数据存放到baos中
    ...
    fun compressBitmapToFile(bmp: Bitmap, file: File) {
        // 尺寸压缩倍数,值越大，图片尺寸越小
        val ratio = 8
        // 压缩Bitmap到对应尺寸
